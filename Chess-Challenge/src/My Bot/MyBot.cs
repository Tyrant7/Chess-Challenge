...
public class MyBot : IChessBot
{
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        int Evaluate()
        {
            // Returns a PeSTO value with baked piece value for a given piece type
            // after the appropriate square has been loaded into the PSTBits buffer
            int GetPieceValue(int type) =>
                (byte)(PSTBits[type >> 2] >> ((type & 3) << 3)) + (int)(0x01283211C521F823 >> type % 6 * 10 & 0x3FF);
            ...
            for (; --sideToMove >= 0; middlegame = -middlegame, endgame = -endgame)
                ...
                for (piece = -1; ++piece < 6;)
                    for (ulong mask = board.GetPieceBitboard((PieceType)piece + 1, sideToMove > 0); mask != 0;)
                    {
                        ...
                        // Index, flip for white
                        square = BitboardHelper.ClearAndGetIndexOfLSB(ref mask) ^ 56 * sideToMove;

                        // Load our bits for this square into the buffer to be read from when retrieving PST values
                        decimal.GetBits(PackedPestoTables[square], PSTBits);

                        // Material and square evaluation
                        middlegame += GetPieceValue(piece);
                        endgame += GetPieceValue(piece + 6);
